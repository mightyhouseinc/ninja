...
host = Platform(options.host) if options.host else platform
...
n.comment(f'It is generated by {os.path.basename(__file__)}.')
n.newline()
...
env_keys = {'CXX', 'AR', 'CFLAGS', 'CXXFLAGS', 'LDFLAGS'}
configure_env = {k: os.environ[k] for k in os.environ if k in env_keys}
if configure_env:
    config_str = ' '.join(
        [f'{k}={pipes.quote(configure_env[k])}' for k in configure_env]
    )
    n.variable('configure_env', f'{config_str}$ ')
n.newline()
...
def cc(name, **kwargs):
    return n.build(built(name + objext), 'cxx', src(f'{name}.c'), **kwargs)
def cxx(name, **kwargs):
    return n.build(built(name + objext), 'cxx', src(f'{name}.cc'), **kwargs)
def binary(name):
...
if platform.is_msvc():
    ...
    cflags = [
        ...
        '/D_HAS_EXCEPTIONS=0',
        f'/DNINJA_PYTHON="{options.with_python}"',
    ]
    ...
    if platform.is_aix():
        cflags.extend(('-D__STDC_FORMAT_MACROS', '-D_LARGE_FILES'))
libs = []
...
def shell_escape(str):
    ...
    if platform.is_windows():
      return str
    return "'%s'" % str.replace("'", "\\'") if '"' in str else str
...
if platform.supports_ninja_browse():
    ...
    n.rule(
        'inline',
        command=f""""{src('inline.sh')}" $varname < $in > $out""",
